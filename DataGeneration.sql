...
begin
	...
	select @begin = '2002-01-01', 
		   @end = GETDATE(),
		   @randReceptionDate = convert(date, cast(cast(@begin as float) + (cast(@end as float) - cast(@begin as float)) * RAND(CHECKSUM(NEWID())) as datetime));

	if @randEmployee in (select EmployeeCode from Career)
	begin
		declare @prevDismissialDate date = (select top(1) DismissalDate from Career where EmployeeCode = @randEmployee order by DismissalDate desc);
		while @randReceptionDate < @prevDismissialDate
		begin
			set @randReceptionDate = convert(date, cast(cast(@begin as float) + (cast(@end as float) - cast(@begin as float)) * RAND(CHECKSUM(NEWID())) as datetime));
		end;
	end;

	set @randDismissialDate = convert(date, cast(cast(@begin as float) + (cast(@end as float) - cast(@begin as float)) * RAND(CHECKSUM(NEWID())) as datetime));
	...
	insert into Career values(@randJob, @randEmployee, @randDepartment, @randReceptionDate, @randDismissialDate);
...
begin
	...
	begin
		...
		set @careerId = (select Code from Career where Code = (select top(1) Code from @codeTable));
		set @minSalary = (select j.MinSalary from Job j left join Career c on j.Code = c.JobCode where c.Code = @careerId);
		...
		begin
			...
			insert into Salary values(@id, @insMonth, @insYear, @minSalary + (rand(checksum(newid())) * 1000));
