...
select j.Name,
	min(s.Salary)
from Salary s 
inner join Career c on c.EmployeeCode = s.EmployeeCode
inner join Job j on c.JobCode = j.Code 
group by j.Name
...
where Code in (select EmployeeCode from Salary where Year = 2015 and Month = 1);
...
where YEAR = 2015 and 
	  ...
	  Salary < any (select Salary 
		from Salary 
		where Year = 2015 and MONTH < 5 and EmployeeCode = sq.EmployeeCode);
...
select d.Code,
	   ...
	   (select count(*) 
			from Career 
			where DepartmentCode = d.Code and DismissalDate is NULL) as Count
 from Department d;


select d.Name, 
		count(c.Code) as [Count] 
from Department d inner join Career c on d.Code = c.DepartmentCode 
where c.DismissalDate is NULL group by d.Name;

-- Group

-- 8) Найти среднюю начисленную зарплату за 2015 год в разрезе работников
select
(select Surname from Employee where Code = s.EmployeeCode) as [Surname],
...
from Salary s 
where Year = 2015 and
(select count(Month) from Salary where EmployeeCode = s.EmployeeCode) > 2 group by EmployeeCode;
...
select e.Surname, e.Name, s.Salary 
from Employee e left join Salary s 
on e.Code = s.EmployeeCode 
where Salary > 1000 and Month = 1 and Year = 2015;
...
select e.Surname as [Surname],
	   ...
	   inner join Employee e on e.Code = c.EmployeeCode
	   inner join Department d on c.DepartmentCode = d.Code
	   inner join Job j on c.JobCode = j.Code;
